    ...
    public function addToCart(Request $request)
{
    ...
    $this->validate($request, [
        'product_id' => 'required|exists:tehs,id', //PASTIKAN PRODUCT_IDNYA ADA DI DB
    ...
    if ($carts && array_key_exists($request->product_id, $carts)) {
        ...
        $product = teh::find($request->product_id);
        //TAMBAHKAN DATA BARU DENGAN MENJADIKAN PRODUCT_ID SEBAGAI KEY DARI ARRAY CARTS
        $carts[$request->product_id] = [
            'qty' => $request->qty,
            'product_id' => $product->id,
            'product_name' => $product->nama_teh,
            'product_price' => $product->harga_teh,
...
public function listCart()
{
    ...
    return view('frontend.ecommerce.cart', compact('carts', 'subtotal'));
}
